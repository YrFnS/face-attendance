...
- version 8 adds immutable verified ERPNext idempotency-contract evidence and permits post-submit replay only for jobs bound to that proof.
...
Frozen synthetic databases for released versions 1–4 live under `tests/fixtures/`. Their manifest records the source commit, released migration checksum, raw database digest, compressed digest, and size. Tests materialize those exact committed bytes and run the normal verified backup-before-migrate path to version 8. See `docs/event-identity-tombstones.md` for the identifier and retention contract, `docs/delivery-outbox.md` for the durable queue, and `docs/erpnext-idempotency.md` for schema 8 and the server contract.
